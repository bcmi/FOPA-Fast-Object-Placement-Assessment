...
def Backbone_ResNet18_in3(pretrained=True):
    if pretrained:
        print("The backbone model loads the pretrained parameters...")
    net = pretrained_resnet18_4ch(pretrained=True)
    div_2 = nn.Sequential(*list(net.children())[:3])
    div_4 = nn.Sequential(*list(net.children())[3:5])
    div_8 = net.layer2
    div_16 = net.layer3
...
def pretrained_resnet18_4ch(pretrained=True, **kwargs):
    ...
    if pretrained:
        checkpoint = torch.load('./data/data/Net_best.pth.tar')
...
def pretrained_resnet50_4ch(pretrained=True, **kwargs):
    model = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    model.conv1 = nn.Conv2d(4, 64, kernel_size=(7, 7), stride=(2, 2), padding=3, bias=False)
    # 加载与训练模型，这里把输入第四个通道的权重权重按照rgb转gray的公式复制了一份
    if pretrained:
        checkpoint = torch.load('/tmp/pcp814/Net_50.pth.tar')
        model.load_state_dict(checkpoint['state_dict'], strict=False)
...
def pretrained_resnet34_4ch(pretrained=True, **kwargs):
    model = ResNet(BasicBlock, [3, 4, 6, 3], **kwargs)
    model.conv1 = nn.Conv2d(4, 64, kernel_size=(7, 7), stride=(2, 2), padding=3, bias=False)
    # 加载与训练模型，这里把输入第四个通道的权重权重按照rgb转gray的公式复制了一份
    if pretrained:
        checkpoint = torch.load('/tmp/pcp814/Net_34_best.pth.tar')
        model.load_state_dict(checkpoint['state_dict'], strict=False)
